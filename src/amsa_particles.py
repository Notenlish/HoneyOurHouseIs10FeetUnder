...
class ParticleBox:
    def __init__(
        ...
        shape: Union[str, pygame.Surface],
        ...
        angle: int,
        shape_width: int = 0,
        color_overtime: Tuple[Tuple[int, int, int, int], Union[int, None]] = None,
        vel_overtime: Tuple[Tuple[int, int], Union[int, None]] = None,
        size_overtime: Tuple[Tuple[int, int], Union[int, None]] = None,
        angle_overtime: Tuple[int, Union[int, None]] = None,
    ...
    def spawn_particle(self):
        shape = (
            random.choice(self.shape)
            if type(self.shape) in (tuple, list)
            else self.shape
        )

        if type(self.shape_width) in (tuple, list):
            if len(self.shape_width) == 3 and self.shape_width[2] == "minmax":
                shape_width = random.randint(
                    min(self.shape_width[0], self.shape_width[1]),
                    max(self.shape_width[0], self.shape_width[1]),
                )
            else:
        ...
        if type(self.color[0]) in (tuple, list):
            if len(self.color) == 3 and self.color[2] == "minmax":
                color = [
                    random.randint(
                        min(self.color[0][0], self.color[1][0]),
                        max(self.color[0][0], self.color[1][0]),
                    ),
                    random.randint(
                        min(self.color[0][1], self.color[1][1]),
                        max(self.color[0][1], self.color[1][1]),
                    ),
                    random.randint(
                        min(self.color[0][2], self.color[1][2]),
                        max(self.color[0][2], self.color[1][2]),
                    ),
                ]
                ...
                if len_c1 == 4 and len_c2 == 4:
                    color.append(
                        random.randint(
                            (min(self.color[0][3], self.color[1][3])),
                            max(self.color[0][3], self.color[1][3]),
                        )
                    )
        ...
        if type(self.lifetime) in (tuple, list):
            if len(self.lifetime) == 3 and self.lifetime[2] == "minmax":
                lifetime = random.uniform(
                    min(self.lifetime[0], self.lifetime[1]),
        ...
        if type(self.vel[0]) in (tuple, list):
            if len(self.vel) == 3 and self.vel[2] == "minmax":
                vel = pygame.Vector2(
                    random.uniform(
                        min(self.vel[0][0], self.vel[1][0]),
                        max(self.vel[0][0], self.vel[1][0]),
                    ),
                    random.uniform(
                        min(self.vel[0][1], self.vel[1][1]),
                        max(self.vel[0][1], self.vel[1][1]),
                    ),
                )
            else:
        ...
        if type(self.size[0]) in (tuple, list):
            if len(self.size) == 3 and self.size[2] == "minmax":
                size = [
                    random.uniform(
                        min(self.size[0][0], self.size[1][0]),
                        max(self.size[0][0], self.size[1][0]),
                    ),
                    random.uniform(
                        min(self.size[0][1], self.size[1][1]),
                        max(self.size[0][1], self.size[1][1]),
                    ),
                ]
            else:
        ...
        if type(self.angle) in (tuple, list):
            if len(self.angle) == 3 and self.angle[2] == "minmax":
                angle = random.uniform(
                    min(self.angle[0], self.angle[1]), max(self.angle[0], self.angle[1])
                )
            else:
        ...
        self.particles.append(
            Particle(
                random.uniform(self.spawn_points[0][0], self.spawn_points[0][1]),
                random.uniform(self.spawn_points[1][0], self.spawn_points[1][1]),
                shape,
                shape_width,
                color,
                lifetime,
        ...
        for particle in reversed(self.particles):
            ...
            if type(particle.shape) == str:
                if particle.shape == "rect":
                    ...
                    if particle.size[0] == particle.size[1]:
                        pygame.draw.circle(
                            ...
                            particle.color,
                            (
                                particle.pos.x + particle.size[0] * 0.5,
                                particle.pos.y + particle.size[1] * 0.5,
                            ),
                            particle.size[0] * 0.5,
                            particle.shape_width,
                        )
                ...
                blits.append(
                    (
                        particle.img,
                        (particle.pos.x - scroll[0], particle.pos.y - scroll[1]),
                    )
                )
            particle.update(dt)
        win.fblits(blits)
    ...
    def __init__(
        ...
        self.curr_overtime = {
            "color": [
                ...
                (
                    self.overtime["color"]
                    if not self.overtime["color"]
                    else self.overtime["color"][0]
                ),
            ],
            "vel": [
                0,
                (
                    self.overtime["vel"]
                    if not self.overtime["vel"]
                    else self.overtime["vel"][0]
                ),
            ],
            "size": [
                0,
                (
                    self.overtime["size"]
                    if not self.overtime["size"]
                    else self.overtime["size"][0]
                ),
            ],
            "angle": [
                0,
                (
                    self.overtime["angle"]
                    if not self.overtime["angle"]
                    else self.overtime["angle"][0]
                ),
            ],
        }
        ...
        if curr_overtime_values[1]:
            if value_type == "angle":
                value_to_change = pygame.math.lerp(
                    orig_value, curr_overtime_values[1][0], currtime_perc
                )
            else:
                value_to_change = [
                    pygame.math.lerp(
                        orig_value[i], curr_overtime_values[1][0][i], currtime_perc
                    )
...
if __name__ == "__main__":
    ...
    pbs = [
        ParticleBox(
            (-10, 540, 800, 60),
            "circle",
            [[x + 128, x + 128, x + 128, 255] for x in range(127)],
            3,
            [(0, -50), (0, -100), "minmax"],
            [15, 15],
            0,
    ...
    while True:
        ...
        for pb in pbs:
            ...
            pb.rect.center = mpos
            pb.spawn_points = [
                [pb.rect.left, pb.rect.right],
                [pb.rect.top, pb.rect.bottom],
            ]
