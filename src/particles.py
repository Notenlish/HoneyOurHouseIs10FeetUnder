    ...
    def spawn_particles(self, verts: list[pygame.Vector2]):
        ...
        for line in lines:
            ...
            while True:
                ...
                particle_pos_new = particle_pos.move_towards(self.orig, 10)
                _dir = particle_pos - particle_pos_new
                if _dir.length():
                    _dir = _dir.normalize()

                self.particles.append((particle_pos_new, _dir))
                point = new.copy()
...
class DustParticles(Particles):
    ...
    def update(self, dt):
        ...
        base = pygame.Color(120, 120, 120)
        addition = pygame.Color([normalised * 50] * 3)
        self.color = base + addition
